...
async function main() {
  ...
  await (await vault.setIsSentinel(owner, true)).wait();
...
*/
  ...
  try {
    ...
    const secondsToAdvance = 4 * 24 * 3600;
...
*/
  ...
  try {
    ...
    try {
      ...
      console.log("✗ Market cap failed:", capErr.message);
    }
       */

  } catch (err: any) {
    console.log("✗ Market configuration failed:", err.message);
  }
